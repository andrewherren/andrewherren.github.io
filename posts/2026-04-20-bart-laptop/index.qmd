...
v3 yields a ~1.8x runtime improvement over v1 on the same benchmark, with a similar small impact on RMSE.
...
When we switch from row-major to column-major storage for the covariates, we obtain essentially no improvement over v3. (Though column-major storage matters much more for the XBART algorithm, which we'll discuss later.)
...
We find that fixing the depth of the trees and the associated storage changes on their own provide almost no performance improvement over v3 and v4 on an apple silicon CPU, but do lead to a minor-but-noticeable worsening of test-set RMSE.
...
This update provides another major computational step change in the BART performance scaffold, bringing total runtime down to 33 seconds for a 15x improvement over the naive implementation.
...
This update does not offer any runtime improvement over v9, perhaps because the compiler is able to easily auto-vectorize the "extra" $O(n)$ residual passes in v9 or perhaps because each sufficient statistic update must access two vectors (the full residual and the prediction cache) which partially offsets the work reduction.
...
This update improves benchmark runtime from 36 seconds to 25 seconds. We note that several BART packages, including `stochtree`, include a variant of this technique.
...
We can see in the table below that both of the R packages attain a slightly lower test-set RMSE, with `XBART` as the faster implementation.
...
for (int i = begin_k; i < end_k; i++) {
  // Get the raw data index of the observtion in node k
...
2. Development roadmap for `faststochtree`:
  * Adding R and Python bindings to the code so that these fast BART / XBART algorithms can be more easily run on real data
...
[^1]: It is fair to say that the algorithm is only viable with pre-sorted covariates
[^2]: The code checks for the availability of NEON instructions with `#ifdef __ARM_NEON`
